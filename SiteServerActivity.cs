        ...
        public override void ProcessActivity(WebRequest request, WebResponse response)
        {
            ...
            switch (model)
            {
                ...
                case "Domain":
                    ...
                    // WebServlet.IsHttps = Domains["scheme"] == "https";
                ...
                case "CSR":
                    {
                        ...
                        var certmodel = this.AsyncDialog("CertModel", rm =>
                        {
                            ...
                            if (form.ContainsKey("limit") == false)
                            {
                                this.Context.Send(new UISectionBuilder(request.Model, request.Command, request.Arguments)
                                    .RefreshEvent($"{request.Model}.{request.Command}")
                                    // .CloseEvent($"{request.Model}.{request.Command}.Del")
                                    .Builder(), true);
                            ...
                            // ui.Delete(UICell.UI("域名", hash["domain"] as string), new UIEventText().Click(new UIClick("Model", "Del", "Domain", host).Send(request.Model, request.Command)));
                            ...
                            if (UMC.Net.Certificater.Certificates.TryGetValue(host, out var _v))
                            {
                                if (_v.Certificate != null)
                                {
                                    ...
                                    if (hash.ContainsKey("expire"))
                                    {
                                        ui.NewSection().AddCell("证书公用名", cn.Substring(3));
                                    }
                                    else
                                    {
                                        ui.NewSection().AddCell("证书公用名", cn.Substring(3)).AddCell("证书过期", Utility.Expire(Utility.TimeSpan(), Utility.TimeSpan(Convert.ToDateTime(_v.Certificate.GetExpirationDateString())), "正签发"));
                                    }
                                }
                            ...
                            {
                                strBtn = "免费申请";
                                // ui.NewSection().AddCell("未有证书", "请申请");
                            ...
                            ui.NewSection().AddCell("过期轮换", hash["contract"] as string);
                            ui.NewSection().AddCell("联系官方", "天才工程师为你解答", new UIClick("Contact").Send("System", "License"));
                            ...
                            ui.UIFootBar.AddText(new UIEventText(strBtn).Click(new UIClick("Model", "ApplyCert", "Domain", hash["domain"] as string).Send(request.Model, request.Command)),
                                    new UIEventText("订购轮换").Click(new UIClick("Model", "Recharge", "Code", hash["root"] as string).Send(request.Model, request.Command)).Style(new UIStyle().BgColor()));
                        ...
                        {
                            case "verifing":
                                webr2.Post(new WebMeta().Put("type", "verify", "domain", host), webr => { });
                                break;
                            case "check":
                ...
                case "Recharge":
                    {
                        var Code = UIDialog.AsyncDialog(this.Context, "Code", g =>
                       {
                        ...
                        var ComboValue = UMC.Data.Utility.IntParse(UMC.Web.UIDialog.AsyncDialog(this.Context, "Combo", gg =>
                        {
                            ...
                            var data = JSON.Deserialize<System.Collections.Hashtable>(webr.Post(new WebMeta().Put("type", "Cert")).ReadAsString());
                            ...
                            var desc = new UMC.Web.WebMeta().Put("title", "证书过期轮换").Put("icon", "\uf0ee");
                            fom.Config.Put("Header", new UIHeader().Desc(desc, "{icon}\n{title}", style));
                            fom.AddTextValue().Put("订阅域名", $"*.{Code}(任意多级)");
